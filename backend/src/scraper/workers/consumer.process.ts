...
export class ConsumerScrapingWorker extends WorkerHost {
  ...
  private async saveData(url: string, data: VehicleOutputDTO) {
    ...
    data.brand.name !== undefined
      ? (brand = await this.brandService.getByName(data.brand.name))
      : (brand = null);

    if (!brand) brand = await this.brandService.create(data.brand);

    let model: Model | null;
    data.model.name !== undefined
      ? (model = await this.modelService.getByNameAndBrand(
          data.model.name,
          brand.id,
        ))
      : (model = null);
    ...
    data.vehicle.year && data.vehicle.version !== undefined
      ? (vehicle = await this.vehicleService.getByYearAndModelAndVersion(
          data.vehicle.year,
          model.id,
