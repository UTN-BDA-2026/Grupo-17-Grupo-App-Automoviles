...
export class DiscoverScrapingWorker extends WorkerHost {
    ...
    private async saveData(links: LinkInput[], storeData: StoreData) {
        ...
        storeData.base_url !== undefined ? store = await this.storeService.getByBaseURL(storeData.base_url) : store = null

        if (!store) store = await this.storeService.create({ name: storeData.store_name, base_url: storeData.base_url })
        ...
        if (links) {
            ...
            for (const link of links) {
                ...
                try {
                    ...
                    this.logger.warn(`The link couldn't be created. Error: ${error.message}`)
                }
            }
