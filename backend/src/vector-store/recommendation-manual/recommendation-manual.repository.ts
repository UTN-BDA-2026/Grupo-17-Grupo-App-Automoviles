...
export class RecommendationManualRepository implements OnModuleInit {
    ...
    async onModuleInit() {
        try {
            ...
            this.logger.log(`Colección "${this.collectionName}" lista (${count} chunks)`);
        } catch (err: any) {
    ...
    async query(queryEmbedding: number[], limit: number, filter?: Record<string, any>): Promise<QueryResult> {
