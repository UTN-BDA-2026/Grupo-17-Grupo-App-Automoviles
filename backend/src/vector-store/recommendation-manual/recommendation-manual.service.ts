...
export class RecommendationManualService {
    ...
    private readonly documentPath = path.join(__dirname, '..', 'documents', 'recommendation_manual.md');
    ...
    async sync(): Promise<SyncResult> {
        ...
        if (!fs.existsSync(this.documentPath)) {
            throw new NotFoundException(`Documento no encontrado en: ${this.documentPath}`);
        }
        ...
        const embeddings = await this.embeddingService.embedDocuments(chunks.map(c => c.content));
        ...
        this.logger.log(`Sincronización completada: ${chunks.length} chunks almacenados`);
    ...
    async store(content: string, metadata: Record<string, any> = {}): Promise<{ id: string }> {
    ...
    private chunkDocument(content: string): DocumentChunk[] {
        ...
        return sections.map(section => {
            ...
            const title = titleMatch ? titleMatch[1].trim() : `Sección ${sectionNumber}`;
