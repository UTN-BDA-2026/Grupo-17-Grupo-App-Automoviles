...
import { JobInfoDTO, QueueStatsDTO, SchedulerStatsDTO } from './dto/job-info.dto'
...
export class SchedulerService {
    ...
    public async consumer() {
        ...
        const pendingLinks = await this.linkService.getByStatus(LinkStatus.PENDIENTE)
        ...
        if (url) {
            try {
                ...
                this.logger.error(`Error adding job to scraping URL ${url}:`, error.message)
            }
        }
    }
    ...
    private async getQueueStats(queue: Queue, name: string): Promise<QueueStatsDTO> {
        const [activeCount, completedCount, failedCount, delayedCount, waitingCount] = 
            await Promise.all([
    ...
    public async getQueueJobsByState(
        ...
        const queue = queueName === 'discover-vehicles' ? this.discoverQueue : this.scrapingQueue;
    ...
    public async getJobDetail(queueName: string, jobId: string): Promise<JobInfoDTO | null> {
        const queue = queueName === 'discover-vehicles' ? this.discoverQueue : this.scrapingQueue;
